...
public class ChatController {
    ...
    public ChatController(@Qualifier("deepResearch") StateGraph stateGraph, SearchBeanUtil searchBeanUtil,
        ...
        SaverConfig saverConfig =
            SaverConfig.builder().register(SaverEnum.MEMORY.getValue(), new MemorySaver()).build();
        this.compiledGraph = stateGraph.compile(
            CompileConfig.builder().saverConfig(saverConfig).interruptBefore("human_feedback").withLifecycleListener(
                    new GraphObservationLifecycleListener(observationRegistry.getIfUnique(() -> ObservationRegistry.NOOP)))
                .build());
     ...
     *
     * 接受ChatRequest并返回一个Flux，该Flux以ServerSentEvent<String>的形式流式传输聊天响应。 支持初始问题和人类反馈的处理。
     */
    ...
    public Flux<ServerSentEvent<String>> chatStream(@RequestBody(required = false) ChatRequest chatRequest)
        ...
        // 创建一个单播接收器以发送ServerSentEvents
        Sinks.Many<ServerSentEvent<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

        // 如果自动接受被禁用且提供了反馈，则处理人类反馈
        ...
        return sink.asFlux().doOnCancel(() -> logger.info("Client disconnected from stream"))
            .onErrorResume(throwable -> {
                ...
                return Mono.just(ServerSentEvent.<String>builder().event("error")
                    .data("Error occurred during streaming: " + throwable.getMessage()).build());
            });
    }
